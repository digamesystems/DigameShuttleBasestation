...
void eInkManager(void *parameter){
  ...
  for(;;){   
    ...
    if (countsChanged(currentShuttleStop) || displayUpdateNeeded) {
...
void showCountDisplay(ShuttleStop &shuttleStop){
  ...
  n = sprintf(stopInfo1, "In : %d",
            shuttleStop.counterEvents[SHUTTLE][INBOUND]);

  n = sprintf(stopInfo2, "Out: %d",
            shuttleStop.counterEvents[SHUTTLE][OUTBOUND]);

  if (shuttleStop.location == reportingLocation){
    displayTextScreenLarge(reportingLocation, stopInfo1, stopInfo2);
  } else {
    displayTextScreenLarge(shuttleStop.location.c_str(), stopInfo1, stopInfo2);
  }
